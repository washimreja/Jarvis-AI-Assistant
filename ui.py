...
class MainWindow(QMainWindow):
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By Washim Reja", C.PRI_DIM))
        return w
    ...
    def resizeEvent(self, event):
        ...
        overlays = [
            ...
            getattr(self, "_plugin_manager_overlay", None),
            getattr(self, "_plugin_settings_overlay", None),
            getattr(self, "_confirm_overlay", None),
        ]
        for ov in overlays:
            if ov is not None and ov.isVisible():
                ow = ov.width()
                oh = min(ov.height(), max(100, cw.height() - 16))
                ov.setGeometry(
                    max(0, (cw.width() - ow) // 2),
                    max(0, (cw.height() - oh) // 2),
                    ow, oh,
                )
        if hasattr(self, "_position_clipboard_panel"):
            self._position_clipboard_panel()
